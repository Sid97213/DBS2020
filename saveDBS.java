...
class Relations{
	...
	public static List<String> candidate_key(String relation,String fd)
	{
        ...
        for(int i=0; i<=(1<<n); i++){                               //creating all combinations of attr
            for(int j=0; j<n; j++){
                if ((i & (1 << j)) != 0){
    ...
    public static void convert_to_bcnf(Relations r){
        ...
        if(!is_bcnf){
            ...
            while(!is_bcnf){
                ...
                Set<Character> union = new HashSet<Character>(alpha);           //Relation 1: (alpha U beta)
                union.addAll(beta);
                ...
                Set<Character> difference = new HashSet<Character>(beta);       //Relation 2: (R-(beta-alpha))
                difference.removeAll(alpha); 
                Set<Character> difference1 = new HashSet<Character>(r_set); 
                difference1.removeAll(difference); 
                //List<String> attr = convertSetToList(difference1);
